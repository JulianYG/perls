...
robotCamCornerPoints = np.hstack((np.array(robotCamCornerPoints[0]).reshape((54, 2))[::-1], 
    np.zeros((54, 1), dtype=np.float32)))

def calibrate_transformation(boardShape, robotCamCornerPoints):
...
tvec, invRotation = calibrate_transformation((9, 6), robotCamCornerPoints)
