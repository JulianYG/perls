...
class IKeyboard(CtrlInterface):
	...
	def _remote_comm(self, model):
		...
		pos = [model.get_tool_pose(t)[0] for t in tool]
		pseudo_event = {0: 0}
		...
		while True:
			if model.controllers:
				...
				for e in events:
					...
					self._control_handler(e, model, control_map, pseudo_event, pos)
	...
	def _local_comm(self, model):
		...
		while True:
			events = p.getKeyboardEvents()
			self._control_handler(events, model, control_map, pseudo_event, pos)	
			time.sleep(0.01)

	def _control_handler(self, events, model, control_map, pseudo_event, pos):
		...
		for e in (events):
			...
			if 99 in events and (events[99] == p.KEY_IS_DOWN):
				model.grip(control_map[1][pseudo_event[0]])

			if 114 in events and (events[114] == p.KEY_IS_DOWN):
				model.release(control_map[1][pseudo_event[0]])
