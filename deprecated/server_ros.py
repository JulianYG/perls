...
from VRcontrol import RobotController
...
p.resetSimulation()
urdf = os.path.abspath(os.path.join(os.getcwd(), '../perls/data/sawyer_robot/sawyer_description/urdf/sawyer_arm.urdf'))

sawyer = p.loadURDF(urdf, (0, 0, 0.9), useFixedBase=1)
p.loadURDF(os.path.abspath(os.path.join(os.getcwd(), '../perls/data/plane.urdf')))
...
class VRSawyer(object):
	...
	def turn_on(self):
		...
		self.arm_initial_pos = np.array(list(self._arm.tool_pose[0]))
	...
	def _event_handler(self, msg):
		...
		if self.engaged:
			...
			self._arm_target_pos = self.arm_initial_pos + rel_pos
			...
			for i in range(7):
				p.setJointMotorControl2(sawyer, i,
					p.POSITION_CONTROL,
					targetVelocity = 0,
					...
					positionGain=0.5,
					velocityGain = 1.)
				p.resetJointState(sawyer, i, jpos[i], 0)
