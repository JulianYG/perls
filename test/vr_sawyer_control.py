...
LOWER_LIMITS = [-3.0503,  -3.8183, -3.0514, 
	-3.0514, -2.9842, -2.9842, -4.7104]
UPPER_LIMITS = [3.0503,  2.2824, 3.0514, 
	3.0514, 2.9842, 2.9842, 4.7104]
...
sawyer = p.loadURDF("sawyer_robot/sawyer_description/urdf/sawyer_arm.urdf", [0.5, -0.3,
	0], useFixedBase=True)

# p.resetBasePositionAndOrientation(sawyer, [0.5, -1, 0.912989899704], [0, 0, 0, 1])
p.resetBasePositionAndOrientation(sawyer, [0.5, -1, 0.91], [0, 0, 0, 1])
...
for jointIndex in range (p.getNumJoints(sawyer)):
	...
	p.setJointMotorControl2(sawyer, jointIndex, p.POSITION_CONTROL, 
		targetPosition=REST_POSE[jointIndex], targetVelocity=0, 
...
sawyer_cid = p.createConstraint(sawyer, 6, sawyer_gripper, 0, p.JOINT_FIXED, 
...
while True:
	...
	for e in (p.getVREvents()):
		...
		eef_pos = e[POSITION]
		sq_len = euc_dist(p.getLinkState(sawyer, 6)[0], eef_pos)

		if sq_len < THRESHOLD * THRESHOLD:
			
			joint_pos = p.calculateInverseKinematics(sawyer, 6, eef_pos, (0, 1, 0, 0),
				lowerLimits=LOWER_LIMITS, upperLimits=UPPER_LIMITS, 
				jointRanges=JOINT_RANGE, restPoses=REST_POSE, jointDamping=JOINT_DAMP)

			for jointIndex in range(p.getNumJoints(sawyer)):
				p.setJointMotorControl2(sawyer, jointIndex, p.POSITION_CONTROL, 
						targetPosition=joint_pos[jointIndex], targetVelocity=0, 
						positionGain=0.05, velocityGain=1.0, force=MAX_FORCE)
