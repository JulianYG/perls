...
class Kinect(Camera):
    ...
    def __init__(self, dimension='hd', 
        ...
        assert dimension in self._DIM_MAP.keys()
    ...
    def turn_on(self):
        ...
        device = self._fn.openDevice(self._serial, pipeline=pipeline)
    ...
    def snapshot(self):
        ...
        color, ir, depth = self._frames['color'], self._frames['ir'], self._frames['depth']
    ...
    def stream(self):
        ...
        while True:
            ...
            cv2.namedWindow('kinect-bigdepth', cv2.CV_WINDOW_AUTOSIZE)
            bigdepth = cv2.resize(self._bigdepth.asarray(np.float32), (int(1920 / 3), int(1082 / 3)))
            cv2.imshow("kinect-bigdepth", bigdepth)
            cv2.setMouseCallback('kinect-registered', mouse_callback, bigdepth)
