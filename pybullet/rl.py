...
class BulletPhysicsVR(object):
	...
	def __init__(self, pybullet, gripper_id, task=0, hand=False):
		...
		try:
			...
			while True:
				...
				# Removing this line for video generation may sync the process?
					...
					for gripper_id in self.GRIPPER_ID:
						gripper_info = [gripper_id] + list(self.p.getBasePositionAndOrientation(gripper_id)[0]) + \
							list(self.p.getBasePositionAndOrientation(gripper_id)[1])
	...
	def replay(self, file, saveVideo=0):
		...
		for row in reader:
			if int(row[0]) == -1:
				...
				if int(row[0]) not in self.GRIPPER_ID:
					self.p.resetBasePositionAndOrientation(int(row[0]) - max(self.GRIPPER_ID) - 1, (float(row[1]), float(row[2]), 
					...
					joint_pos = self.p.calculateInverseKinematics(self.ROBOT_MAP[int(row[0])], 6, eef_pos, eef_orien)
					self.p.resetBasePositionAndOrientation(self.ROBOT_MAP[int(row[0])] + 1, eef_pos, eef_orien)
	...
	def _init_task(self):
		...
		repo[1] = [(0, 3), ("sphere_small.urdf",0.80000,-0.200000,0.699990,0.000000,0.0,0.00000,1),
